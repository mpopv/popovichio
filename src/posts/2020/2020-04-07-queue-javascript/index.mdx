...
Firefox and Safari seem to handle repeated unshifts in a much more performant way under the hood than Chrome, so the performance test at the end is best viewed in Chrome! Otherwise the browser optimizes the operations so that both data structures are about even.
