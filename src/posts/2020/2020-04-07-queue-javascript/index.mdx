...
Firefox and Safari handle `shift`/`unshift` in a much more performant way under the hood than Chromium, so the performance test at the end is best viewed in Chrome or Edge! Otherwise the browser optimizes the operations so that both data structures are about even. (See [here](https://lannonbr.com/blog/2020-01-27-shift-optimizations/) for more how they were able to optimize slow array methods.)
